...
function createEvent(item) {
    ...
    if (item.alldayevent == true) {
        ...
        event += "DTSTART;TZID=America/New_York:" + formatDateTime(item.start) + "\n";
        event += "DTEND;TZID=America/New_York:" + formatDateTime(item.end) + "\n";
    }
...
var o = new ActiveXObject("Outlook.Application");
var session = o.Session
var oNS = o.GetNameSpace("MAPI");
//var oNS = o.Session;
var oCalendar = oNS.GetDefaultFolder(olFolderCalendar);
var oCalItems = oCalendar.Items();


	var today = new Date();
	var total = oCalItems.Count;
	var exportItem = true;
	var item;
	...
	for (var i=1; i<=total; i++) {
		...
		try {
			var rItem = oCalendar(i);
			item = new ActiveXObject("Redemption.SafeContactItem");
			item.item = rItem;
			includeBody = true;
		} catch(e) {
			item = oCalItems(i);
